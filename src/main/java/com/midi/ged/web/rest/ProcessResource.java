...
/**
 * REST controller for managing {@link com.midi.ged.domain.Process}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class ProcessResource {
    ...
    public ResponseEntity<Process> partialUpdateProcess(
        @PathVariable(value = "id", required = false) final Long id,
        @NotNull @RequestBody Process process
    ) throws URISyntaxException {
        ...
        Optional<Process> result = processRepository
            ...
            .map(existingProcess -> {
                if (process.getCode() != null) {
                    existingProcess.setCode(process.getCode());
                }
                if (process.getNom() != null) {
                    existingProcess.setNom(process.getNom());
                }
